...
def get_args():
    ...
    parser.add_argument("-p", "--in_dir", help="The directory where the pp files are stored without yearly subfolder")
    ...
    parser.add_argument("-m", "--month", nargs='+', type=int, help="The month(s) you want to calculated CAPE")
...
def cape_calculate(f):
    wrf_nc = Dataset(f)
    cape_2d = wrf.getvar(wrf_nc, "cape_2d", timeidx=wrf.ALL_TIMES).isel(mcape_mcin_lcl_lfc=0, Time=np.arange(0, 24))
    wrf_nc.close()
...
def copy_one_day(fg_dir, working_dir, date):
    source_dir = path.join(fg_dir, datetime.strftime(date, "%Y"), datetime.strftime(date, "%Y-%m"),
...
def write_cape_to_nc(cape_data, sample_file, out_file):
    ...
    dataset.DATA_NOTES = "File generated with the output of successive model " \
                         "runs of 36H (first 12 hours discarded for spin-up)"
    dataset.WRF_VERSION = ds.attrs["TITLE"].split()[2] + " " + ds.attrs["TITLE"].split()[3]
    dataset.CREATED_BY = "Xun Wang - xun.wang@tu-berlin.de"
    dataset.INSTITUTION = "Technische Universitaet Berlin, Institute of Ecology, Chair of Climatology"
    now = datetime.now()
    ...
    dataset.LEVEL_INFO = "H: original simulation output; D: daily; M: monthly; Y: yearly; S: static"
    ...
    dataset.GRID_INFO = "Grid spacing: GRID_DX and GRID_DY (unit: m), " \
    ...
    west_east = dataset.createDimension('west_east', ds.west_east.size)
    south_north = dataset.createDimension('south_north', ds.south_north.size)
    ...
    times.units = "hour since " + str(dates[0])
    ...
    # for coordinates
    dates_new = []
    for n in range(0, cape_data.cape_2d.shape[0]):
        dates_new.append(datetime(dates.year[0], dates.month[0], dates.day[0]) + n * timedelta(hours=1))
